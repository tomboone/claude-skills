...
`/implement` already ends with its own `/code-review --fix` pass. ADR 0002 argued that the post-ship `/personal:reviewit` pass earned its keep as "the public, durable gate." In practice the loop's economics changed:
...
The pre-ship `/code-review --fix` inside `/implement` stays, and ADR 0002's closing instruction still holds with more force than before: **do not remove it.** It is now the only code review in the loop.
